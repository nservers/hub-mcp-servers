...
test('ecommerce-mcp boots up, responds to healthcheck with 200, and enforces Bearer auth on /sse', async (t) => {
  const env = {
    ...process.env,
    PORT: '3993',
    HOST: '127.0.0.1',
    MCP_AUTH_TOKEN: 'test_secret_ecommerce_token_444',
  };
  ...
  const resHealth = await fetch('http://127.0.0.1:3993/health');
  assert.strictEqual(resHealth.status, 200);
  const dataHealth = await resHealth.json();
  assert.strictEqual(dataHealth.service, 'ecommerce-mcp');
  ...
  const resUnauth = await fetch('http://127.0.0.1:3993/sse');
  ...
  const resWrong = await fetch('http://127.0.0.1:3993/sse', {
  ...
  const resAuth = await fetch('http://127.0.0.1:3993/sse', {
    headers: { Authorization: 'Bearer test_secret_ecommerce_token_444' },
