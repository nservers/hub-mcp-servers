...
test('cloud-connect-mcp boots up, responds to healthcheck with 200, and enforces Bearer auth on /sse', async (t) => {
  const env = {
    ...process.env,
    PORT: '3996',
    HOST: '127.0.0.1',
    NSERVERS_API_TOKEN: 'mock_token',
    MCP_AUTH_TOKEN: 'test_secret_cloud_token_777',
  };
  ...
  const resHealth = await fetch('http://127.0.0.1:3996/health');
  assert.strictEqual(resHealth.status, 200);
  const dataHealth = await resHealth.json();
  assert.strictEqual(dataHealth.service, 'cloud-connect-mcp');
  ...
  const resUnauth = await fetch('http://127.0.0.1:3996/sse');
  ...
  const resWrong = await fetch('http://127.0.0.1:3996/sse', {
  ...
  const resAuth = await fetch('http://127.0.0.1:3996/sse', {
    headers: { Authorization: 'Bearer test_secret_cloud_token_777' },
