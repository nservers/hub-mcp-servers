...
test('redis-mcp boots up, responds to healthcheck, and enforces Bearer auth on /sse', async (t) => {
  const env = {
    ...process.env,
    PORT: '3998',
    HOST: '127.0.0.1',
    ...
    REDIS_PORT: '6379',
    MCP_AUTH_TOKEN: 'test_secret_redis_mcp_token_999',
  };
  ...
  const resHealth = await fetch('http://127.0.0.1:3998/health');
  assert.strictEqual(resHealth.status, 503);
  const dataHealth = await resHealth.json();
  assert.strictEqual(dataHealth.service, 'redis-mcp');
  assert.strictEqual(dataHealth.version, '1.0.0');

  // SSE endpoint without auth (expect 401 Unauthorized)
  const resUnauth = await fetch('http://127.0.0.1:3998/sse');
  ...
  const resWrong = await fetch('http://127.0.0.1:3998/sse', {
  ...
  const resAuth = await fetch('http://127.0.0.1:3998/sse', {
    headers: { Authorization: 'Bearer test_secret_redis_mcp_token_999' },
