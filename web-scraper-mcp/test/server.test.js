...
test('web-scraper-mcp boots up, responds to healthcheck with 200, and enforces Bearer auth on /sse', async (t) => {
  const env = {
    ...process.env,
    PORT: '3992',
    HOST: '127.0.0.1',
    MCP_AUTH_TOKEN: 'test_secret_scraper_token_333',
  };
  ...
  const resHealth = await fetch('http://127.0.0.1:3992/health');
  assert.strictEqual(resHealth.status, 200);
  const dataHealth = await resHealth.json();
  assert.strictEqual(dataHealth.service, 'web-scraper-mcp');
  ...
  const resUnauth = await fetch('http://127.0.0.1:3992/sse');
  ...
  const resWrong = await fetch('http://127.0.0.1:3992/sse', {
  ...
  const resAuth = await fetch('http://127.0.0.1:3992/sse', {
    headers: { Authorization: 'Bearer test_secret_scraper_token_333' },
