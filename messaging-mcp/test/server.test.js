...
test('messaging-mcp boots up, responds to healthcheck with 200, and enforces Bearer auth on /sse', async (t) => {
  const env = {
    ...process.env,
    PORT: '3991',
    HOST: '127.0.0.1',
    MCP_AUTH_TOKEN: 'test_secret_messaging_token_222',
  };
  ...
  const resHealth = await fetch('http://127.0.0.1:3991/health');
  assert.strictEqual(resHealth.status, 200);
  const dataHealth = await resHealth.json();
  assert.strictEqual(dataHealth.service, 'messaging-mcp');
  ...
  const resUnauth = await fetch('http://127.0.0.1:3991/sse');
  ...
  const resWrong = await fetch('http://127.0.0.1:3991/sse', {
  ...
  const resAuth = await fetch('http://127.0.0.1:3991/sse', {
    headers: { Authorization: 'Bearer test_secret_messaging_token_222' },
